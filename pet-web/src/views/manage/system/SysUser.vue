<template>
    <div>
        <div class="search">
            <el-input placeholder="请输入姓名查询" style="width: 200px" v-model="nickName"></el-input>
        ...
        <div class="operation">
            <el-button type="primary" icon="el-icon-plus" plain @click="handleAdd">新增用户</el-button>
        </div>
            ...
            <el-table :data="tableData">
                ...
                <el-table-column label="操作" width="300" align="center" fixed="right">
                    <template v-slot="scope">
                        <el-button plain type="primary" icon="el-icon-setting" @click="updateRoleData(scope.row)" size="mini"
                                   :disabled="scope.row.userId === 1 ">分配角色
                        </el-button>
                        <el-button plain type="primary" icon="el-icon-edit" @click="handleEdit(scope.row)" size="mini"
                                   :disabled="scope.row.userId === 1 ">编辑
                        </el-button>
        ...
        <el-dialog title="用户信息" :visible.sync="fromVisible" width="55%" :close-on-click-modal="false" destroy-on-close>
            <el-form :inline="true" label-width="120px" style="padding: 0px 50px" :model="form" :rules="rules" ref="formRef" size="small">
                ...
                <el-form-item label="职位" prop="job">
                    <el-input v-model="form.job" placeholder="请输入职位" autocomplete="off" />
                </el-form-item>
                ...
                <el-form-item label="擅长" prop="beGoodAt">
                    <el-select v-model="form.beGoodAt" placeholder="请选择" multiple>
                        <el-option
                                v-for="item in goodAt"
                                :key="item.value"
                                :label="item.label"
                                :value="item.value">
                        </el-option>
...
export default {
    ...
    methods: {
        load(pageNum) {  // 分页查询
            ...
            this.$request.get('/sysuser/selectPage', {
                params: {
        ...
        save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
            this.$refs.formRef.validate((valid) => {
                if (valid) {
                    this.$request({
                        url: this.form.userId ? '/sysuser/updateById' : '/sysuser/add',
        ...
        del(userId) {   // 单个删除
            this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
                this.$request.delete('/sysuser/delete/' + userId).then(res => {
        ...
        //查询所有角色
        getRoleAllList() {
            this.$request.get('/sysrole/selectAll',{
                params: {
                    status: "0",
                }
            }).then(res => {
