<template>
    <div>
            ...
            <el-table :data="tableData" stripe  @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="img" label="封面" align="center">
                    <template v-slot="scope">
                        <el-image :preview-src-list="[scope.row.img]" :src="scope.row.img" style="width: 60px; height: 40px;"></el-image>
                    </template>
                </el-table-column>
        ...
        <el-dialog title="分类信息" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
        ...
        methods: {
            ...
            save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
                this.$refs.formRef.validate((valid) => {
                    if (valid) {
                        ...
                        this.$request({
                            url: this.form.id ? '/buscategory/update' : '/buscategory/add',
            ...
            del(id) {   // 单个删除
                this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
                    this.$request.delete('/buscategory/delete/' + id).then(res => {
            ...
            delBatch() {   // 批量删除
                ...
                this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
                    this.$request.delete('/buscategory/delete/batch', {data: this.ids}).then(res => {
                        if (res.code === '200') {   // 表示操作成功
                            this.$message.success('操作成功')
                            this.load(1)
                        } else {
            ...
            load(pageNum) {  // 分页查询
                ...
                this.$request.get('/buscategory/selectPage', {
                    params: {
