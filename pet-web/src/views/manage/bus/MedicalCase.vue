<template>
    <div>
        <div class="search">
            <el-input placeholder="请输入宠物名称查询" style="width: 200px" v-model="name"></el-input>
        ...
        methods: {
            ...
            save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
                this.$refs.formRef.validate((valid) => {
                    if (valid) {
                        this.$request({
                            url: this.form.id ? '/medicalCase/update' : '/medicalCase/add',
            ...
            delBatch() {   // 批量删除
                ...
                this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
                    this.$request.delete('/medicalCase/delete/batch', {data: this.ids}).then(res => {
                        if (res.code === '200') {   // 表示操作成功
                            this.$message.success('操作成功')
                            this.load(1)
                        } else {
            ...
            load(pageNum) {  // 分页查询
                ...
                this.$request.get('/medicalCase/selectPage', {
                    params: {
